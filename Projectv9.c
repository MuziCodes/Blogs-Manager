...
int display(char *usernamefile){                   //put a fflush after every input
...
void compose(char *usernamefile){
    ...
    do{
        printf("Enter the body (press Enter twice to complete the input): ");     //changes
        count = 0;
        verify = 1;
        while(1){
            ch1 = getchar();
...
char *Sign(int mode){
    ...
    char *usernameptr;          //so that it won't be destroyed when function ends
    ...
    if(mode == 1){
        do{
            ...
            fflush(stdin);
            usernameptr = (char *)calloc( (strlen(username)+5), sizeof(char) );    //We added +5 so null terminator and .txt will also be included
            strcpy(usernameptr, username);

            strcat(usernameptr, ".txt");
            strcat(usernameptr, "\0");

            fpointer = fopen(usernameptr, "r");
